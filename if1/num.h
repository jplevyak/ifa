...
enum IF1_const_kind { IF1_CONST_KIND_STRING = IF1_NUM_KIND_COMPLEX + 1, IF1_CONST_KIND_SYMBOL, IF1_CONST_KIND_BYTES };
...
inline unsigned int ImmHashFns::hash(Immediate *imm) {
  ...
  for (int i = 0; i < (int)(sizeof(*imm) / sizeof(unsigned int)); i++)
    h = h + open_hash_primes[i] * ((unsigned int *)imm)[i];
  return h;
}
...
int sprint_imm(char *s, size_t size, Immediate &imm);
