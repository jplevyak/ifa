...
void
PCallbacks::new_SUM_type(Sym *s) {
  assert(s->type_kind == Type_SUM);
}

Sym *
PCallbacks::new_Sym(cchar *name) {
  Sym *sy = new Sym;
...
Prim *find_prim(ParseAST *ast) {
  ...
  if (!prim) {
    ...
    if (ast->children[0]->sym && ast->children.v[0]->sym->is_symbol) {
      ...
      if (!prim)
        return 0;
      if (!compatible_type(prim->arg_types[0], ast->children[1]->sym))
        return 0;
    }
    else {
      ...
      if (!prim)
        return 0;
      if (!compatible_type(prim->arg_types[0], ast->children[0]->sym))
        return 0;
      if (nargs > 0)
...
static Sym *
new_constant(IF1 *i, cchar *string, cchar *constant_type) {
...
Sym *
new_sym(IF1 *i, Scope *scope, cchar *s, Sym *sym) {
  if (!sym)
    sym = new_Sym(s);
  if (!sym->in && scope) {
...
static void
set_builtin(IF1 *i, Sym *sym, cchar *start, cchar *end = 0) {
...
#undef S
  ...
  switch (x) {
    ...
    case Builtin_tuple: sym->type_kind = Type_RECORD; break;
    case Builtin_void: sym->type_kind = Type_RECORD; break;
    case Builtin_ref: 
...
static Sym * 
make_module(IF1 *i, cchar *mod_name, Scope *global, Sym *sym = 0) {
...
static Sym *
in_module(IF1 *i, cchar *mod_name, Scope *scope, Sym *sym = 0) {
...
resolve_parameterized_type(IF1 *i, ParseAST *ast) {
  ...
  if (ast->children.n > 1) {
    ...
    for (int i = 1; i < ast->children.n; i++) {
      ...
      if (!(sym = checked_ast_qualified_ident_sym(ast->children[i]->get(AST_qualified_ident))))
        return -1;
      s->has.add(sym);
    }
    ast->sym = s;
...
define_function(IF1 *i, ParseAST *ast) {
  ...
  for (int x = 1; x < ast->children.n - 1; x++)
    if (scope_pattern(i, ast->children[x], fn->scope) < 0)
      return -1;
  fn->is_fun = 1;
...
static int
get_apply_args(IF1 *i, Code **c, ParseAST *ast, Vec<Sym *> &args, Vec<cchar *> &arg_names) {
...
gen_op(IF1 *i, ParseAST *ast) {
  ...
  if (ast->is_comma)
    ...
    if (binary)
      send = if1_send(i, c, 5, 1, sym_primitive, sym_make_tuple, aa0, ast->children[ast->op_index]->rval, 
                      aa1, args);
    else if (a0)
      send = if1_send(i, c, 4, 1, sym_primitive, sym_make_tuple, aa0, ast->children[ast->op_index]->rval, args);
    else
      send = if1_send(i, c, 4, 1, sym_primitive, sym_make_tuple, ast->children[ast->op_index]->rval, aa1, args);
    send->ast = ast;
...
gen_constructor(IF1 *i, ParseAST *ast) {
  ...
  if (ast->kind == AST_vector)
    if1_add_send_arg(i, send, int32_constant(ast->rank));
  for (int x = 0; x < args.n; x++)
    if1_add_send_arg(i, send, args[x]);
...
gen_def_ident(IF1 *i, ParseAST *ast) {
  ...
  for (int x = 1; x < ast->children.n; x++)
    if (ast->children[x] != must_implement &&
        ast->children[x] != must_specialize &&
        ast->children[x] != pattern &&
        ast->children[x] != var) {
      val = ast->children[x];
      break;
    }
