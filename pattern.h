...
class MPosition : public gc {
 public:
  ...
  void set_top(const void *p) { pos[pos.n-1] = p; cp = 0; }
  void push(int i) { 
  ...
  void push(const void *p) { pos.add(p); cp = 0; }
  ...
  int is_positional() { 
    for (int i = 0; i < pos.n; i++) 
      if (!is_intPosition(pos[i])) return 0; return 1; 
  }
...
int pattern_match(Vec<AVar *> &args, Vec<cchar *> &names, AVar *send, int is_closure, 
                  Partial_kind partial, PNode *visibility_point, Vec<Match *> &matches);
