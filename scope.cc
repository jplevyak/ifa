...
void Scope::add_dynamic(Scope *s, Sym *sy) {
  dynamic.add(s);
...
Sym *Scope::get_local(cchar *name, Sym **container) {
  Sym *s;
  if ((s = hash.get(name))) {
...
Sym *Scope::get(cchar *name, Sym **container) {
  Sym *s;
  if ((s = get_local(name, container))) return s;
  if (up) return up->get(name, container);
  return NULL;
}
...
Scope *Scope::function() {
  ...
  if (s->in->is_module) s = s->in->init->scope;
  return s;
}

void Scope::put(cchar *name, Sym *s) {
