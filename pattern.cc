...
int
positional_to_named(PNode *pn, CreationSet *cs, Vec<cchar *> &names, 
  ...
  if (*funs) {
    forv_Fun(f, **funs) if (f) {
      ...
      for (int i = 0; i < unused_acpps.n; i++) {
        ...
        if (unused_acpps[i] != fcpp)
          mapped_positions.put(unused_acpps[i], 1);
        fpp.inc();
      }
    }
  }
...
void
Matcher::find_all_matches(CreationSet *cs, Vec<AVar *> &args, Vec<cchar *> &names,
...
class DefaultCacheHashFns  {
 public:
  static unsigned int hash(Vec<MPosition*> *a) {
    unsigned int h = 0, x = 0;
    forv_MPosition(p, *a)
      h += open_hash_primes[x++ % 256] * (uintptr_t)a;
    return h;
  }
...
make_generic_substitution(IFAAST *ast, Sym *generic_type, Sym *concrete_type, Map<Sym *, Sym *> &substitutions) {
  ...
  if (unify_instantiated(generic_type, concrete_type, subs)) {
    for (int i = 0; i < subs.n; i++)
      if (!make_generic_substitution(ast, subs[i].key, subs.v[i].value, substitutions))
        return 0;
  }
...
verify_arg(Sym *actual, CreationSet *cs, PMatch *m, MPosition *fcpp) { 
  ...
  if (formal_type != actual_type && !formal_type->specializers.set_in(actual_type->type))
    return 0;
  return 1;
}
                            ...
                            Vec<Fun *> &result, int top_level)
{
  ...
  for (int i = 0; i < covered.n; i++) {
    if (!generic_substitutions(&covered[i], app, csargs, ast))
      continue;
    promotion_uses(&covered[i], app, csargs);
    coercion_uses(&covered[i], app, csargs);
    if (!verify_args(covered[i], app, csargs))
      continue;
  ...
  for (int i = 0; i < applicable.n; i++) if (applicable[i]) {
    if (subsumed.set_in(applicable[i]))
      continue;
    for (int j = i + 1; j < applicable.n; j++) if (applicable[j]) {
      switch (subsumes(applicable[i], applicable.v[j], app, csargs)) {
        case -1: subsumed.set_add(applicable[j]); break;
        case 0: break;
...
static void
get_all_args(AVar *a, MapMPositionAType &all_args, MapMPositionAVar &all_avars, Vec<MPosition *> &ps, MPosition &p) {
...
static void
get_all_args(Vec<AVar *> &args, MapMPositionAType &all_args, MapMPositionAVar &all_avars, Vec<MPosition *> &ps) {
  MPosition p;
  p.push(1);
  forv_AVar(a, args) {
...
match_cache_hit(Vec<AVar *> &args, AVar *send, int is_closure, Partial_kind partial, 
  ...
  forv_MatchCacheEntry(e, entries) {
    ...
    form_MPositionAType(x, e->all_args)
      all_avars.get(x->key)->arg_of_send.add(send);
    forv_Match(mm, *e) {
...
//
int
pattern_match(Vec<AVar *> &args, Vec<cchar *> &names,
