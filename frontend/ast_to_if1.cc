...
static int compatible_type(PrimType pt, Sym *s) {
  switch (pt) {
    ...
    case PRIM_TYPE_ANY_INT_B:
      if (s && s->type && (s->type->num_kind == IF1_NUM_KIND_INT || s->type->num_kind == IF1_NUM_KIND_UINT)) return 1;
      break;
    default:
      assert(!"case");
      break;
...
Prim *find_prim(ParseAST *ast) {
  ...
  if (!prim) {
    ...
    if (ast->children[0]->sym && ast->children.v[0]->sym->is_symbol) {
      prim = if1->primitives->prim_map[nargs][0].get(ast->children[0]->sym->name);
      if (!prim) return 0;
      if (!compatible_type(prim->arg_types[0], ast->children[1]->sym)) return 0;
    } else {
      assert(ast->children[1]->sym && ast->children.v[1]->sym->is_symbol);
      prim = if1->primitives->prim_map[nargs][1].get(ast->children[1]->sym->name);
      if (!prim) return 0;
      if (!compatible_type(prim->arg_types[0], ast->children[0]->sym)) return 0;
      if (nargs > 0)
        if (!compatible_type(prim->arg_types[1], ast->children[2]->sym)) return 0;
    }
    assert(prim);
  }
...
static void dump_ast_tree(FILE *fp, Fun *f, ParseAST *a, int indent = 0) {
  ...
  if (a->children.n) {
    for (int i = 0; i < indent; i++) putc(' ', fp);
    fprintf(fp, "<UL>\n");
    for (ParseAST *aa : a->children) dump_ast_tree(fp, f, aa, indent + 1);
    for (int i = 0; i < indent; i++) putc(' ', fp);
    fprintf(fp, "</UL>\n");
  }
}
...
static void graph_ast_nodes(FILE *fp, ParseAST *a) {
  ...
  for (ParseAST *aa : a->children) if (graph_it(aa)) graph_ast_nodes(fp, aa);
}

static void graph_ast_edges(FILE *fp, ParseAST *a) {
  for (ParseAST *aa : a->children) if (graph_it(aa)) {
    graph_edge(fp, a, aa);
...
static void dig_ast(ParseAST *ast, D_ParseNode *pn) {
  if (pn->user.ast)
    ...
    for (int i = 0; i < d_get_number_of_children(pn); i++) dig_ast(ast, d_get_child(pn, i));
}
...
IFAAST *ParseAST::copy_tree(ASTCopyContext *context) {
  ...
  for (int i = 0; i < a->children.n; i++) a->children[i] = (ParseAST *)a->children.v[i]->copy_tree(context);
  return a;
}
...
void ParseAST::add_below(D_ParseNode *pn) {
  for (int i = 0; i < d_get_number_of_children(pn); i++) dig_ast(this, d_get_child(pn, i));
}

static inline ParseAST *ast_qualified_ident_ident(ParseAST *x) { return x->last(); }
...
static Scope *ast_qualified_ident_scope(ParseAST *a) {
  ...
  for (int x = i; x < a->children.n - 1; x++) {
    ...
    if (!sym || !sym->scope) {
      show_error("unresolved identifier qualifier '%s'", a, a->children[x]->string);
      return 0;
    }
    s = sym->scope;
  }
...
void ast_print_recursive(FILE *fp, ParseAST *a, int indent) {
  ...
  for (ParseAST *aa : a->children) ast_print_recursive(fp, aa, indent + 1);
}
...
static void build_builtin_syms(IF1 *i, ParseAST *ast) {
  ...
  for (ParseAST *a : ast->children) build_builtin_syms(i, a);
}
...
static void build_constant_syms(IF1 *i, ParseAST *ast) {
  switch (ast->kind) {
    case AST_const:
      ...
      if (!ast->sym) ast->sym = new_constant(i, ast->string, ast->constant_type);
      break;
    default:
      break;
  }
  for (ParseAST *a : ast->children) build_constant_syms(i, a);
}
 ...
 */
static int define_types(IF1 *i, ParseAST *ast, Vec<ParseAST *> &funs, Scope *scope, int skip = 0) {
  if (!skip) {
    ast->scope = scope;
    switch (ast->kind) {
      case AST_in_module: {
      ...
      case AST_def_type: {
        ...
        if (!sym) {
          ...
          if (ast->sym) return show_error("duplicate identifier '%s'", ast, ast->get(AST_ident)->string);
          ast->sym = sym;
        }
        ...
        if (ifa_verbose > 2) printf("creating scope %X for %s\n", (int)(intptr_t)ast->sym->scope, ast->sym->name);
      ...
      case AST_record_type:
        if (!ast->sym) {
          ...
          ast->sym->ast = ast;
          if (ast->kind == AST_record_type) scope = ast->sym->scope = new Scope(scope, Scope_RECURSIVE, scope->in);
        } else
          ast->sym->type_kind = ast_to_type(ast);
        break;
      case AST_loop:
...
static int resolve_parameterized_type(IF1 *i, ParseAST *ast) {
  Sym *sym;
  if (!(sym = checked_ast_qualified_ident_sym(ast->get(AST_qualified_ident)))) return -1;
  if (ast->children.n > 1) {
    Sym *s = new_sym(i, ast->scope);
    s->type_kind = Type_APPLICATION;
    s->has.add(sym);
    for (int i = 1; i < ast->children.n; i++) {
      assert(ast->children[i]->kind == AST_type_param && ast->children.v[i]->sym);
      if (!(sym = checked_ast_qualified_ident_sym(ast->children[i]->get(AST_qualified_ident)))) return -1;
      s->has.add(sym);
    }
    ast->sym = s;
  } else
...
static void set_scope_recursive(ParseAST *ast, Scope *scope) {
  ast->scope = scope;
  for (ParseAST *a : ast->children) set_scope_recursive(a, scope);
}
...
static int scope_pattern(IF1 *i, ParseAST *ast, Scope *scope) {
  switch (ast->kind) {
    case AST_pattern: {
      ...
      ast->sym->ast = ast;
      for (ParseAST *a : ast->children) if (a != ptype) {
    ...
    case AST_rest: {
      ...
      if (!var && !ast->sym->is_symbol && !ast->sym->is_constant) ast->sym->is_local = 1;
      break;
    }
    default:
      break;
...
static int scope_inherits(ParseAST *ast, Sym *sym) {
  for (ParseAST *a : ast->children) {
    if (a->kind == AST_includes || a->kind == AST_inherits) {
      if (!(a->sym = checked_ast_qualified_ident_sym(a->get(AST_qualified_ident)))) return -1;
...
static int resolve_types_and_define_recursive_functions(IF1 *i, ParseAST *ast, int skip = 0) {
  if (!skip) switch (ast->kind) {
      ...
      case AST_with: {
        ...
        for (ParseAST *a : with_scope->children) {
          if (!a->sym)
            if (!checked_ast_qualified_ident_sym(a)) return -1;
          if (!a->sym->must_specialize) show_error("with without declared type", ast);
  ...
  for (ParseAST *a : ast->children) if (resolve_types_and_define_recursive_functions(i, a) < 0) return -1;
  return 0;
}
...
static int variables_and_nonrecursive_functions(IF1 *i, ParseAST *ast, int skip = 0) {
  if (!skip) switch (ast->kind) {
      case AST_def_ident: {
  ...
  for (ParseAST *a : ast->children) if (variables_and_nonrecursive_functions(i, a) < 0) return -1;
  return 0;
}
...
static int build_types(IF1 *i, ParseAST *ast) {
  for (ParseAST *a : ast->children) if (build_types(i, a) < 0) return -1;
  switch (ast->kind) {
    case AST_type_param:
      ast->sym = ast->children[0]->sym;
      break;
    ...
    case AST_type_application:
    case AST_record_type:
      for (ParseAST *a : ast->children) if (a->sym) {
        switch (a->kind) {
          case AST_type_param:
            ast->sym->has.add(a->sym);
            break;
      ...
      break;
    case AST_sum_type:
      for (ParseAST *a : ast->children) if (a->sym) a->sym->inherits_add(ast->sym);
      break;
    case AST_def_type: {
    ...
    case AST_must_implement:
      for (ParseAST *a : ast->children) if (!ast->sym) ast->sym = a->sym;
      break;
    default:
      break;
  }
...
static int unalias_types(IF1 *i, ParseAST *ast) {
  for (ParseAST *a : ast->children) if (unalias_types(i, a) < 0) return -1;
...
static int set_scope_kind(IF1 *i, ParseAST *ast, int scope_kind) {
  ...
  for (ParseAST *a : ast->children) if (set_scope_kind(i, a, scope_kind) < 0) return -1;
  return 0;
}
...
static int define_labels(IF1 *i, ParseAST *ast, LabelMap *labelmap) {
  ...
  for (ParseAST *a : ast->children) if (define_labels(i, a, labelmap) < 0) return -1;
  return 0;
}

static int resolve_labels(IF1 *i, ParseAST *ast, LabelMap *labelmap, Label *break_label = 0, Label *continue_label = 0,
  ...
  switch (ast->kind) {
    ...
    case AST_return:
      ...
      break;
    case AST_op:
      if (ast->children[ast->op_index]->sym->name[0] == ',') ast->children[0]->in_tuple = 1;
      if (ast->children[ast->op_index]->sym->name[0] == '^' && ast->children[ast->op_index]->sym->name[1] == '^')
        ast->children[0]->in_apply = 1;
      break;
    case AST_scope:
      for (ParseAST *a : ast->children) a->constructor =
          a->scope->kind == Scope_PARALLEL ? Make_VECTOR : (a->scope->kind == Scope_SEQUENTIAL ? Make_TUPLE : Make_SET);
      break;
    default:
      break;
  }
  for (ParseAST *a : ast->children) if (resolve_labels(i, a, labelmap, break_label, continue_label, return_label) < 0) return -1;
  return 0;
}
...
static int get_tuple_args(IF1 *i, Code **c, ParseAST *ast, Vec<Sym *> &args) {
  if (ast->kind == AST_op && ast->children[ast->op_index]->sym->name[0] == ',') {
...
static int get_apply_args(IF1 *i, Code **c, ParseAST *ast, Vec<Sym *> &args, Vec<cchar *> &arg_names) {
...
static void gen_new(IF1 *i, ParseAST *ast) {
  Code **c = &ast->code;
  for (ParseAST *a : ast->children) if1_gen(i, c, a->code);
  ast->rval = new_sym(i, ast->scope);
...
static void gen_comma_op(IF1 *i, ParseAST *ast, ParseAST *a0, ParseAST *a1) {
  ...
  for (Sym *a : args) if1_add_send_arg(i, send, a);
...
static void gen_apply_op(IF1 *i, ParseAST *ast, ParseAST *a0, ParseAST *a1) {
  ...
  Code *send = if1_send1(i, c);
  send->ast = ast;
  for (int x = 0; x < args.n; x++) if1_add_send_arg(i, send, args[x], arg_names.v[x]);
...
static void gen_op(IF1 *i, ParseAST *ast) {
  ...
  if (ast->is_comma)
    ...
    if (a0->rval->is_read_only) show_error("assignment to read-only symbol", ast);
    ...
    if (binary)
      send =
          if1_send(i, c, 6, 1, sym_primitive, sym_make, sym_tuple, aa0, ast->children[ast->op_index]->rval, aa1, args);
    else if (a0)
      send = if1_send(i, c, 5, 1, sym_primitive, sym_make, sym_tuple, aa0, ast->children[ast->op_index]->rval, args);
    else
      send = if1_send(i, c, 5, 1, sym_primitive, sym_make, sym_tuple, ast->children[ast->op_index]->rval, aa1, args);
    send->ast = ast;
...
static void gen_loop(IF1 *i, ParseAST *ast) {
  ...
  if1_loop(i, &ast->code, ast->label[0], ast->label[1], cond->rval, before ? before->code : 0, cond->code, 0,
           body->code, ast);
  ast->rval = body->rval;
}
...
static void gen_if(IF1 *i, ParseAST *ast, int is_expr) {
  ...
  ast->rval = new_sym(i, ast->scope);
  if1_if(i, &ast->code, ifcond->code, ifcond->rval, ifif->code, ifif->rval, ifelse ? ifelse->code : 0,
         ifelse ? ifelse->rval : 0, is_expr ? ast->rval : 0, ast);
...
static void gen_constructor(IF1 *i, ParseAST *ast) {
  ...
  if (ast->kind == AST_index && ast->children.n < 3) if1_add_send_arg(i, send, int32_constant(1));
  if1_add_send_result(i, send, ast->rval);
}

static int define_type_init(IF1 *i, ParseAST *ast, Sym **container_scope, Sym **container) {
...
static int pre_gen_top_down(IF1 *i, ParseAST *ast, Sym *container_scope, Sym *container = 0) {
  switch (ast->kind) {
    default:
      break;
    case AST_def_type:
  ...
  for (ParseAST *a : ast->children) if (pre_gen_top_down(i, a, container_scope, container) < 0) return -1;
  return 0;
}

static int pre_gen_bottom_up(ParseAST *ast) {
  for (ParseAST *a : ast->children) if (pre_gen_bottom_up(a) < 0) return -1;
  switch (ast->kind) {
    case AST_vector: {
      uint rank = 0;
      for (ParseAST *a : ast->children) if (a->rank > rank) rank = a->rank;
      if (ast->children.n > 1)
        ast->rank = rank + 1;
      else
        ast->rank = rank;
    ...
    case AST_op: {
      ...
      ast->is_assign = (op[1] == '=' && op[0] != '=' && op[0] != '<' && op[0] != '>' && op[0] != '!') || ast->is_inc_dec;
      ast->is_ref = op[0] == '&' && ast->op_index == 0;
      ast->is_application = (op[0] == '^' && op[1] == '^') || op[0] == '(';
      ast->is_comma = op[0] == ',';
      if (ast->is_comma) {
        uint rank = 1;
        for (ParseAST *a : ast->children) if (!a->in_tuple) {
...
static void gen_assign(IF1 *i, ParseAST *ast, ParseAST *val, Sym *in, Sym *out) {
  Sym *args = new_sym(i, ast->scope);
  Code *send = if1_send(i, &ast->code, 5, 1, sym_make, sym_tuple, in, sym_assign, val->rval, args);
  send->ast = ast;
...
static void gen_def_ident_value(IF1 *i, ParseAST *ast, ParseAST *constraint, ParseAST *val) {
...
static Sym *gen_container(IF1 *i, ParseAST *ast) {
  Sym *rval = new_sym(i, ast->scope);
  Code *send =
      if1_send(i, &ast->code, 4, 1, sym_operator, ast->container, sym_period, if1_make_symbol(i, ast->sym->name), rval);
  send->ast = ast;
  rval->is_lvalue = 1;
  return rval;
}
...
static void gen_def_ident(IF1 *i, ParseAST *ast) {
  ...
  for (int x = 1; x < ast->children.n; x++)
    if (ast->children[x] != must_implement && ast->children[x] != must_specialize && ast->children[x] != pattern &&
        ast->children[x] != var) {
      val = ast->children[x];
      break;
    }
...
static int gen_if1(IF1 *i, ParseAST *ast) {
  // bottom's up
  for (ParseAST *a : ast->children) if (gen_if1(i, a) < 0) return -1;
  switch (ast->kind) {
    case AST_def_type:
      gen_type(i, ast);
      break;
    ...
    case AST_scope:
    case AST_block:
      for (ParseAST *a : ast->children) if1_gen(i, &ast->code, a->code);
...
static Sym *collect_module_init(IF1 *i, ParseAST *ast, Sym *mod) {
  for (ParseAST *a : ast->children) switch (a->kind) {
    case AST_in_module:
      mod = a->sym->init;
      break;
    default:
...
static void build_init(IF1 *i) {
  ...
  for (Sym *s : i->allsyms) if (s->is_module) {
...
static void finalize_sym(Sym *f, Sym *s) {
  ...
  if (s->is_pattern) for (Sym *ss : s->has) finalize_sym(f, ss);
}

static void finalize_fun_symbols(Sym *f, Code *code) {
  for (Sym *s : code->lvals) finalize_sym(f, s);
  for (Sym *s : code->rvals) finalize_sym(f, s);
  for (Code *c : code->sub) finalize_fun_symbols(f, c);
}

static void finalize_symbols(IF1 *i) {
  for (Sym *f : i->allclosures) {
    finalize_fun_symbols(f, f->code);
    for (Sym *s : f->has) finalize_sym(f, s);
...
int ast_constant_fold(IF1 *i, ParseAST *ast) {
  for (ParseAST *a : ast->children) if (ast_constant_fold(i, a) < 0) return -1;
  switch (ast->kind) {
    case AST_const:
...
int ast_gen_if1(IF1 *i, Vec<ParseAST *> &av) {
  ...
  for (ParseAST *a : av) build_builtin_syms(i, a);
#define S(_n) assert(sym_##_n);
#include "builtin_symbols.h"
#undef S
  if1_set_primitive_types(i);
  for (ParseAST *a : av) build_constant_syms(i, a);
  ...
  for (ParseAST *a : av) if (build_scopes(i, a, scope) < 0) return -1;
  for (ParseAST *a : av) if (build_types(i, a) < 0) return -1;
  for (ParseAST *a : av) if (unalias_types(i, a) < 0) return -1;
  finalize_types(i);
  for (ParseAST *a : av) if (ast_constant_fold(i, a) < 0) return -1;
  for (ParseAST *a : av) if (build_functions(i, a, user_mod) < 0) return -1;
  build_modules(i);
  build_init(i);
  finalize_symbols(i);
...
void PCallbacks::finalize_functions() {
  ...
  for (Fun *fun : pdb->funs) fun->is_eager = 1;
}
