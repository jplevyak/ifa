...
initialize() {
  forv_Fun(f, fa->funs)
    forv_Var(v, f->fa_all_Vars)
      for (int i = 0; i < v->avars.n; i++) if (v->avars[i].value)
        if (v->avars[i].key && v->avars.v[i].value->contour == GLOBAL_CONTOUR)
...
determine_basic_clones(Vec<Vec<CreationSet *> *> &css_sets_by_sym) {
  ...
  for (int i = 0; i < css_sets_by_sym.n; i++) {
    for (int j = 0; j < css_sets_by_sym[i]->n; j++) {
      if (!css_sets_by_sym[i]->v[j])
        continue;
      for (int k = 0; k < j; k++) {
        if (!css_sets_by_sym[i]->v[k])
          continue;
...
static int
compute_member_types(Vec<CreationSet *> *eqcss) {
  ...
  for (int i = start; i < n; i++) {
    Vec<Sym *> t;
    Sym *s = (i < 0 ? sym->element : sym->has[i]);
    forv_CreationSet(cs, *eqcss) if (cs) {
      AVar *av = i < 0 ? get_element_avar(cs) : cs->vars[i];
      if (!s) {
        if (i < 0) {
          if (!sym->element)
            s = sym->element = new_Sym();
          s = sym->element;
        } else {
          if (!sym->has[i])
            sym->has[i] = new_Sym();
...
static int
concretize_var_list_type(Var *v) {
  Sym *sym = 0;
  ...
  if (all_list) {
    ...
    if (etype->n == 1)
      ...
      forv_CreationSet(cs, *etype) if (cs)
        t->has.set_add(cs->type);
      if (!(v->type = if1->callback->make_LUB_type(t)))
        return -1;
    }
    return 1;
  }
...
concretize_var_type(Var *v) {
  ...
  int ret = 0;
  if ((ret = concretize_var_list_type(v)) != 0)
    return ret;
  for (int i = 0; i < v->avars.n; i++) {
    if (!v->avars[i].key)
      continue;
...
resolve_concrete_types(CSSS &css_sets) {
  for (int i = 0; i < css_sets.n; i++) {
    ...
    forv_CreationSet(cs, *eqcss) if (cs) {
      ...
      if (eav) if (concretize_avar(eav) < 0) return -1;
    }
    Sym *sym = eqcss->first_in_set()->type;
...
clone_functions() {
  ...
  forv_Fun(f, fa->funs)
    for (int i = 0; i < f->calls.n; i++) if (f->calls[i].key)
      f->calls[i].value->set_to_vec();
