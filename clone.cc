...
static void initialize() {
  for (Fun *f : fa->funs) for (Var *v : f->fa_all_Vars) for (int i = 0; i < v->avars.n;
  ...
  for (Sym *s : fa->pdb->if1->allsyms) {
    if (s->creators.n) {
      ...
      for (CreationSet *cs : creators) if (fa->css_set.set_in(cs)) s->creators.add(cs);
    }
  }
}
...
static int prim_period_offset(PNode *p, EntrySet *es) {
  ...
  int offset = -1;
  for (CreationSet *sel : *selector->out) if (sel) {
...
inline int ES_FN::equivalent(EntrySet *a, EntrySet *b) {
  ...
  if (nesting_depth) {
    ...
    for (AEdge *ee : a->edges) if (ee) if (a->display[nesting_depth - 1]->equiv)
        aparents.add(a->display[nesting_depth - 1]->equiv);
    for (AEdge *ee : b->edges) if (ee) if (b->display[nesting_depth - 1]->equiv)
    ...
    if (done.set_add(x)) {
      ...
      for (C *y : aset) if (y) {
        if (!done.set_in(y) && FN::equivalent(x, y)) {
          for (C *z : *equiv) if (z && !FN::equivalent(z, y)) goto Lskip;
          done.set_add(y);
          equiv->set_add(y);
        }
      Lskip:;
...
static void determine_basic_clones(Vec<Vec<CreationSet *> *> &css_sets_by_sym) {
  ...
  for (int i = 0; i < css_sets_by_sym.n; i++) {
    ...
    for (CreationSet *cs : *s) if (cs) cs->equiv = s;
  }
}
...
static void determine_clones() {
  ...
  while (changed_css.n) {
    ...
    for (Fun *f : fa->funs) {
      for (Vec<EntrySet *> *ess : f->equiv_sets) {
        Vec<AEdge *> calls_edges;
        Vec<Vec<AEdge *> *> edge_sets;
        for (EntrySet *es : *ess) if (es) calls_edges.set_union(es->out_edges);
          ...
          for (int j = 0; j < edge_sets[i]->n; j++)
            for (int k = 0; k < j; k++) {
              ...
              if (e1 && e2 && e1->match->fun == e2->match->fun && e1->to->equiv != e2->to->equiv) {
                for (MPosition *p : e1->match->fun->positional_arg_positions) {
                  ...
                  for (CreationSet *c1 : d1) {
                    for (CreationSet *c2 : d2) {
    ...
    if (sym != (Sym *)-1) {
      if (sym != sym_tuple && sym != sym_closure && !tup) {
        ...
        if (!sym->creators.some_difference(creators)) {
          for (CreationSet *cs : *eqcss) if (cs) cs->type = sym;
          continue;
        }
      }
    }
  ...
  for (int i = 0; i < css_sets_local.n; i++) {
    ...
    if (sym != (Sym *)-1) {
      ...
      if (sym == sym_tuple || sym == sym_closure || tup) {
        ...
        for (CreationSet *cs : *eqcss) if (cs) cs->type = s;
      } else {
...
static int resolve_concrete_types(CSSS &css_sets) {
  for (int i = 0; i < css_sets.n; i++) {
    Vec<CreationSet *> *eqcss = css_sets[i];
    for (CreationSet *cs : *eqcss) if (cs) {
      for (AVar *av : cs->vars) if (concretize_avar(av) < 0) return -1;
    ...
    switch (sym->type_kind) {
      ...
      case Type_APPLICATION:
        break;
      case Type_SUM: {
        for (CreationSet *cs : *eqcss) if (cs) sym->has.set_add(cs->sym->type);
        sym->has.set_to_vec();
...
static int concretize_types(Fun *f) {
  for (Var *v : f->fa_all_Vars) if (concretize_var_type(v) < 0) return -1;
  return 0;
}
...
static int clone_functions() {
  ...
  for (Fun *f : fa->funs) for (int i = 0; i < f->calls.n; i++) if (f->calls[i].key) f->calls[i].value->set_to_vec();
  // invert Fun::calls to generated Fun::called
  for (Fun *f : fa->funs) {
    for (int i = 0; i < f->calls.n; i++)
      if (f->calls[i].key) {
        for (Fun *ff : *f->calls[i].value) ff->called.add(new CallPoint(f, f->calls[i].key));
      }
  }
  return 0;
}
...
void log_test_fa(FA *fa) {
  ...
  for (Var *v : gvars) if ((!v->sym->is_constant || !v->sym->constant) && !v->sym->is_symbol && !v->sym->type_kind &&
