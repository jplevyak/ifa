...
static void
remove_unreachable(Fun *f, Vec<PNode *> &nodes) {
  Accum<PNode *> v;
  if (!f->entry)
    return;
  v.add(f->entry);
  for (int i = 0; i < v.asvec.n; i++)
    forv_PNode(p, v.asvec[i]->cfg_succ) if (p)
      v.add(p);
  Vec<PNode *> unreachable;
  forv_PNode(n, nodes)
    if (!v.asset.set_in(n))
      unreachable.add(n);
  forv_PNode(n, unreachable) 
    forv_PNode(s, n->cfg_succ) s->cfg_pred.remove(n);
}
