...
static inline int maybe_live(PNode *n, Var *v) {
  return n->live_vars->get(v) != 0;
}

static int merge_live(PNode *n, PNode *p) {
  int changed = 0;
  forv_Var(v, *p->live_vars) if (v) if (!n->lvals.in(v)) changed =
      !n->live_vars->put(v);
  return changed;
}
...
static void approximate_liveness(Fun *f, Vec<PNode *> &nodes) {
  ...
  while (changed) {
    ...
    forv_PNode(n, nodes) {
      ...
      forv_Var(v, n->rvals) if (v->sym->is_local) changed |=
          !n->live_vars->put(v);
    }
  }
}
...
static void rename_edge(Fun *f, PNode *d, VarEnv &env, Vec<PNode *> &nset) {
  ...
  for (int i = 0; i < d->cfg_succ.n; i++) {
    ...
    forv_PNode(p, dd->phi) p->rvals[di] =
        get_Var(p->rvals[0]->sym->var, env, f);
...
static void rename_vars(Fun *f, Vec<PNode *> nodes) {
  forv_PNode(n, nodes) for (int i = 0; i < n->cfg_pred.n; i++)
