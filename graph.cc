...
static FILE *
graph_start(cchar *fn, cchar *tag, cchar *name) {
  char hfn[512];
  strcpy(hfn, fn); 
  strcat(hfn, ".");
  strcat(hfn, tag);
...
void
graph_node(FILE *fp, void *id, cchar *label, int options) {
  switch (graph_type) {
    case VCG:
...
static void 
graph_ast(Vec<Fun *> &funs, cchar *fn) {
  ...
  if (options & G_DOM) {
    for (int i = 0; i < pn->dom->intervals.n; i += 2) {
      sprintf(title + strlen(title), "[%d %d]", 
              pn->dom->intervals[i], pn->dom->intervals.v[i + 1]);
    }
  }
  if (options & G_LOOP) {
...
static void 
graph_cfg(Vec<Fun *> &funs, cchar *fn) {
...
static void 
graph_dom(Vec<Fun *> &funs, cchar *fn) {
...
static void 
graph_loops(Vec<Fun *> &funs, cchar *fn) {
...
static void 
graph_ssu(Vec<Fun *> &funs, cchar *fn) {
...
static void
graph_abstract_types(FA *fa, cchar *fn) {
