...
Prim::Prim(int aindex, cchar *astring, cchar *aname, int anargs, int apos,
           int anrets, PrimType *aarg_types, PrimType *aret_types,
           int options) {
  index = aindex;
  string = astring;
  name = aname;
...
Primitives::Primitives(IF1 *if1) { prim_init(this, if1); }
...
Prim *Primitives::find(Code *c) {
  if (c->kind != Code_SEND || c->rvals.n < 2) return 0;
  return find(c->rvals.n, c->rvals[0], c->rvals[1],
              c->rvals.n < 3 ? 0 : c->rvals[2]);
}

Prim *Primitives::find(PNode *p) {
  if (p->rvals.n < 2) return 0;
  return find(p->rvals.n, p->rvals[0]->sym, p->rvals[1]->sym,
              p->rvals.n < 3 ? 0 : p->rvals[2]->sym);
}

RegisteredPrim *prim_reg(cchar *name, PrimitiveTransferFunctionPtr ptr,
                         PrimitiveCGPtr pcg) {
