...
static void
dump_header(FILE *fp, cchar *fn, char *mktree_dir) {
  fprintf(fp, "<HTML>\n");
  fprintf(fp, "<HEAD>\n");
  ...
  fprintf(fp, "<ul>\n");
  fprintf(fp, "<li><a href=\"#CONCRETE_TYPES\">Concrete Types (Clones/Instantiations)</a>\n");
  fprintf(fp, "<li><a href=\"#CONCRETE_FUNCTIONS\">Concrete Functions (Clones/Instantiations)</a>\n");
#if FUNCTION_SYMBOLS
...
static int has_no_out_edges(Sym *s) {
  if (s->var)
    form_AVarMapElem(p, s->var->avars)
      if (p->value->forward.n || p->value->arg_of_send.asvec.n)
        return 0;
  return 1;
}
...
static void
dump_sub_sym(FILE *fp, Sym *ss, cchar *title) {
  if (ss && ss->type) {
...
static void
dump_sym_list(FILE *fp, Sym *s, Vec<Sym *> &l, cchar *title, sym_pred_fn fn = 0) {
  if (l.n) {
    Vec<Sym *> v;
...
static void
dump_var_type(FILE *fp, Var *v, int &wrote_one) {
  if (wrote_one)
    fprintf(fp, ", ");
  wrote_one = 1;
...
dump_var_type_list(FILE *fp, Vec<Var *> &vars) {
  ...
  for (int i = 0; i < vars.n; i++)
    dump_var_type(fp, vars[i], wrote_one);
}
...
static void
dump_functions(FILE *fp, Vec<Sym *> funs) {
  fprintf(fp, "<H1><A NAME=\"CONCRETE_FUNCTIONS\">Concrete Functions (Clones/Instantiations)</A></H1>\n\n");
  forv_Sym(fs, funs) {
    Fun *f = fs->fun;
    const char *name = f->sym->name ? f->sym->name : ANON;
    const char *sname = f->sym->in ? f->sym->in->name : "";
    ...
    for (int i = 0; i < f->calls.n; i++)
      if (f->calls[i].value)
        funs.set_union(*f->calls[i].value);
    funs.set_to_vec();
    ...
    forv_Var(v, all_vars)
      if (v->sym->name && v->live && v->type && v->type->name && !v->sym->is_fun && 
          !v->sym->is_symbol && !v->sym->is_constant && v->sym->type_kind == Type_NONE)
        vars.add(v);
    if (vars.n) {
    ...
    if (f->ast) {
      ...
      f->ast->html(fp, f);
      fprintf(fp, "</ul>\n");
      fprintf(fp, "<a href=\"#\" class=\"button\" onClick=\"expandTree('funtree%d'); return false;\">[Expand AST]</a>&nbsp;&nbsp;\n", f->id);
      fprintf(fp, "<a href=\"#\" class=\"button\" onClick=\"collapseTree('funtree%d'); return false;\">[Collapse AST]</a>\n", f->id);
    }
    fprintf(fp, "</TABLE><br>\n");
  }
}
...
dump_symbols(FILE *fp, FA *fa) {
  ...
  forv_CreationSet(cs, fa->css)
    if (cs->type && !cs->type->is_symbol && !cs->type->is_fun &&
  ...
  forv_Sym(s, syms) if (s)
    if (s->name && !s->is_constant && !s->is_fun && !s->is_symbol && !has_no_out_edges(s) && !s->type_kind != Type_NONE && !s->is_local)
      globals.set_add(s);
  globals.set_to_vec();
  ...
  // Concrete Types
  fprintf(fp, "<H1><A NAME=\"CONCRETE_TYPES\">Concrete Types (Clones/Instantiations)</A></H1>\n\n");
...
void 
dump_html(FA *fa, cchar *fn, char *mktree_dir) {
  char hfn[512];
