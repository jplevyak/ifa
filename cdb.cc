...
static cchar *cdb_filename(FA *fa) {
  (void)fa;
  return "ifa.cdb";
}

static int read_cdb_function(FA *fa, FILE *fp) {
  char id[1024];
  int prof_id, nes;
  if (fscanf(fp, "%1023s %d %d", id, &prof_id, &nes) != 3) return -1;
  id[1023] = 0;
  ...
  for (int i = 0; i < nes; i++) {
    int esid;
    if (fscanf(fp, "%d", &esid) != 1) return -1;
    f->prof_ess.add(esid);
  }
  return 0;
}
...
static int read_cdb_entry_set(FA *fa, FILE *fp) {
  ...
  for (int i = 0; i < ncsids; i++) {
    int csid;
    if (fscanf(fp, "%d", &csid) != 1) return -1;
    esinfo->cs_ids.add(csid);
  }
  for (int i = 0; i < nedges; i++) {
    char pnode_id[1024];
    if (fscanf(fp, "%1023s %d", pnode_id, &esid) != 2) return -1;
    pnode_id[1023] = 0;
...
static int read_cdb_creation_set(FA *fa, FILE *fp) {
  char id[1024];
  int csid, ncp;
  if (fscanf(fp, "%1023s %d %d", id, &csid, &ncp) != 3) return -1;
  id[1023] = 0;
  Sym *s = fa->cdb->classid.get(id);
  if (!s) return 0;  // function disappeared
  ...
  if (!v) fa->cdb->cs_info.put(s, (v = new Vec<CDB_CreationSet *>));
  v->add(csinfo);
  return 0;
}

int read_cdb(FA *fa) {
  cchar *fn = cdb_filename(fa);
  FILE *fp = fopen(fn, "r");
  if (!fp) return -1;
  while (1) {
    int c = fgetc(fp);
    ...
    switch (c) {
      default:
        goto Lerror;
      case 'F':
        if (read_cdb_function(fa, fp) < 0) goto Lerror;
        break;
      case 'E':
        if (read_cdb_entry_set(fa, fp) < 0) goto Lerror;
        break;
      case 'C':
        if (read_cdb_creation_set(fa, fp) < 0) goto Lerror;
        break;
    }
  }
  return 0;
